...
Feature: webapp商品列表商品排序
	...
	开启商品排序功能后：默认排序，销量，价格
		...
		3.(商品全部列表和分组商品列表)商品列表默认排序，升序-时间倒序排列
		4.(商品全部列表和分组商品列表)bill第一次点击默认排序，下箭头变红，降序-时间正序排列
		5.(商品全部列表和分组商品列表)bill第二次点击默认排序，上箭头变红，升序-时间倒序排列
		6.(商品全部列表和分组商品列表)bill第一次点击销量排序，下箭头变红，降序-销量由大到小，如：销量一样按照默认排序
		7.(商品全部列表和分组商品列表)bill第二次点击销量排序，上箭头变红，降序-销量由小到大，如：销量一样按照默认排序
