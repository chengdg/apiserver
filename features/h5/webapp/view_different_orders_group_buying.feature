...
Scenario:2 订单列表只有团购订单-团购进行中被自动取消订单
	#未支付，取消的团购活动订单，在手机端不显示
...
Scenario:4 订单列表只有团购订单-团购成团订单发货、完成订单
	#团购成功的订单进行发货和完成订单
