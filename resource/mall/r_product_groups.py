...
class RProductGroup(inner_resource.Resource):
	"""
	商品分组，根据促销活动进行分组
	"""
	app = 'mall'
	...
	@staticmethod
	def group_product_by_promotion(webapp_owner_info, member, products):
		"""
		...
		"""
		member_grade_id, discount = resource.get('member', 'member_discount', {
	    	'member': member,
		...
		for product in products:
			...
			promotion_name = RProductGroup.__get_promotion_name(product)
		...
		for promotion_id, group_info in items:
			...
			group_unified_id = RProductGroup.__get_group_name(products)
			integral_sale_rule = RProductGroup.__collect_integral_sale_rules(member_grade_id, products) if member_grade_id != -1 else None
			...
			# 如果促销对此会员等级的用户不开放
			if not RProductGroup.has_promotion(member_grade_id, promotion.get('member_grade_id')):
				product_groups.append({
				                      "id": group_id,
				                      "uid": group_unified_id,
				                      'products': products,
